...
describe('Tier One: Robots', () => {
	...
	describe('<AllRobots /> component', () => {
		...
		it('*** renders "No Robots" if passed an empty array of robots or if robots is undefined', () => {
			...
			expect(wrapper.text()).to.include('No Robots');
		});
	});
	...
	describe('Sequelize Model', () => {
		...
		it('fuelType can only be gas, diesel, or electric (defaults to electric)', async () => {
			...
			try {
				...
				if (badFuelRobot) {
					throw Error(
						'Validation should have failed with invalid fuelType'
					);
				}
			} catch (err) {
		...
		it('fuelLevel must be between 0 and 100 (defaults to 100)', async () => {
			...
			try {
				...
				if (negativeFuelRobot) {
					throw Error(
						'Validation should have failed with fuelLevel < 0'
					);
				}
			} catch (err) {
				expect(err.message).to.not.have.string(
			...
			try {
				...
				if (tooMuchFuelRobot) {
					throw Error(
						'Validation should have failed with fuelLevel > 100'
					);
				}
			} catch (err) {
				expect(err.message).to.not.have.string(
