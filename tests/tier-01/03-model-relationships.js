...
describe.only('Tier One: Project >-< Robot Association', () => {
	...
	describe('Seed File', () => {
		...
		it('*** creates at least one robot that has several projects', () => {
			const robotWithTwoProjects = robots
				.filter(robot => robot.projects.length > 1)
				.map(robot => robot.name);
			expect(robotWithTwoProjects).to.have.lengthOf.above(0);
		});

		it('*** creates at least one project that has several robots', () => {
			const projectWithTwoRobots = projects
				.filter(project => project.robots.length > 1)
				.map(project => project.title);
			expect(projectWithTwoRobots).to.have.lengthOf.above(0);
		});
	});
});
