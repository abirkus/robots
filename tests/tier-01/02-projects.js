...
describe('Tier One: Projects', () => {
	...
	describe('<AllProjects /> component', () => {
		...
		it('*** renders "No Projects" if passed an empty array of projects or if projects is undefined', () => {
			...
			expect(wrapper.text()).to.include('No Projects');
	...
	describe('Redux', () => {
		...
		describe('projects reducer', () => {
			...
			it('*** returns the initial state by default', () => {
				...
				expect(
					appReducer(testStore.getState(), testRun).projects
				).to.deep.equal([]);
				//throw new Error('replace this error with your own test');
			});
	...
	describe('Connect: react-redux', () => {
		...
		it('<AllProjects /> is passed projects from store as props', async () => {
			...
			const {projects: componentProjects} = wrapper
				.find(AllProjects)
				.props();
	...
	describe('Sequelize Model', () => {
		...
		it('*** deadline must be a valid date', async () => {
			...
			try {
				await project.validate();
				throw Error(
					'validation should have failed with invalid deadline'
				);
			} catch (err) {
				expect(err.message).to.contain(
					'Validation isDate on deadline failed'
				);
			}
		...
		it('priority must be an integer between 1 and 10', async () => {
			...
			try {
				...
				throw Error(
					'validation should have failed with too high priority'
				);
			} catch (err) {
				expect(err.message).to.contain(
					'Validation max on priority failed'
				);
			}
			...
			try {
				...
				throw Error(
					'validation should have failed with too low priority'
				);
			} catch (err) {
				expect(err.message).to.contain(
					'Validation min on priority failed'
				);
			}
		});
